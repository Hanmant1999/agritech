...
cur=con.cursor()
ex="""CREATE TABLE Reg124134(Name VARCHAR(4000),Email VARCHAR(4000),Password VARCHAR(4000));"""
cur.execute(ex)
exa="""CREATE TABLE pri(crop_name VARCHAR(4000),region VARCHAR(4000),price VARCHAR(4000));"""
cur.execute(exa)
K=Tk()
K.geometry("400x400")
...
def search(c,r):
 mb=(c,r)
 cur.execute("""SELECT price FROM pri WHERE crop_name = ? AND region= ?""",mb)
 l=cur.fetchone()
 g=list(l)
 v=messagebox.showinfo("Price",g)
 v.pack()
...
def add():
 ...
 Buty=Button(tab5,text="Done",bg="blue",fg="black",command=lambda:Sam(var.get(),dar.get(),e3.get()))
...
def pag():
 ...
 But16=Button(pag1,text="Add",bg="red",fg="black",command=add) 
 But16.grid(row=4,column=1)
 
 
def check(a,l):
 cur.execute("""SELECT Password FROM Reg124134 WHERE Name = ?""",(a,))
 m=cur.fetchone()
 n=list(m)
 if n[0]==l:
  pag()
...
def reg():
 ...
 e3.grid(row=3,column=1)
 But3=Button(tab2,text="Submit",bg="blue",fg="black",command=lambda:Submit(e.get(),e1.get(),e2.get()))
 But3.grid(row=4,column=1)
 def Submit(m,n,o):
  mk=(m,n,o)
  ex1="""INSERT INTO Reg124134 VALUES(?,?,?);"""
  cur.execute(ex1,mk)
  con.commit()
  home()
